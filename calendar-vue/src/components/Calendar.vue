  ...
  <div class="block block--calendar">
    <div class="calendar">
      ...
      <div class="calendar__legend">
        ...
        <div class="calendar__status">
          <span
            class="calendar__background calendar__background--important"
          ></span>
          Ważny
        </div>
        <div class="calendar__status">
          <span class="calendar__background calendar__background--done"></span>
          Wykonany
        </div>
      </div>
      <fullcalendar
...
export default {
  name: "Calendar",
  data: () => ({
    calendarPlugins: [TimeGrid, DayGrid, InteractionPlugin, ListPlugin],
    calendarHeader: {
  ...
  methods: {
      ...
      axios
        .post("/event", {
          ...
          backgroundColor: this.setBackgroundColor(
            this.$refs["eventModal"].$data.status
          )
        })
        .then(response => {
          this.getEvents();
    ...
    clickEvent(arg) {
      ...
      this.$refs["eventModal"].$data.description =
        arg.event.extendedProps.description;
      this.$refs["eventModal"].$data.status = arg.event.extendedProps.status;
      this.$refs["eventModal"].$data.backgroundColor =
    ...
    edit() {
      ...
      axios
        .put("/event/" + this.event.id, updateEvent)
        .then(response => {
          this.getEvents();
          setTimeout(() => {
            this.$refs["eventModal"].$data.isLoading = false;
    ...
    getEvents() {
      axios
        .get("/calendar/" + this.$route.params.id)
        .then(response => {
